// index.js — CalcNPlay level browser

const VERSION = 'v0.1.0';
const COMMIT_HASH = '6dc9bb3';
const COMMIT_DATE = '2026-05-25';
console.log('%cCalcNPlay ' + VERSION, 'color:#6c5ce7;font-weight:bold;font-size:14px');
...
let chronoEnabled = localStorage.getItem('cn-chrono') === '1';
let audioEnabled  = localStorage.getItem('cn-audio')  !== '0';

// ── Auth & Profiles ───────────────────────────────────────────────────────────

let currentProfileId   = localStorage.getItem('cn-profile-id') || null;
...
function selectProfile(profileId, profileData) {
  ...
  if (profileId) localStorage.setItem('cn-profile-id', profileId);
  else           localStorage.removeItem('cn-profile-id');
...
window.onAuthChanged = user => {
  if (user) {
    ...
    localStorage.removeItem('cn-profile-id');
    showUserAvatar(null);
    const display = document.getElementById('user-display');
...
document.getElementById('chrono-toggle').addEventListener('click', () => {
  ...
  localStorage.setItem('cn-chrono', chronoEnabled ? '1' : '0');
  document.getElementById('chrono-toggle').classList.toggle('on', chronoEnabled);
});

document.getElementById('audio-toggle').addEventListener('click', () => {
  audioEnabled = !audioEnabled;
  localStorage.setItem('cn-audio', audioEnabled ? '1' : '0');
